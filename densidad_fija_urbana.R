# inspirado en https://www.behance.net/gallery/99114047/Population-Density

# gráficos de puntos por habitante con círculos de tamaño fijo, así representan más exactamente la densidad
...
regiones <- readr::read_csv2("datos/cut_regiones.csv")
comunas <- readr::read_csv2("datos/cut_comunas.csv")


# cargar base de datos del censo nivel personas
ruta_censo <- "~/Documents/Datos/Censo/2024"
censo <- open_dataset(file.path(ruta_censo, "personas_censo2024.parquet"))


superficie <- readr::read_csv2("datos/superficies_urbanas_censo_2024.csv")


# calcular ----
# # desde base de datos del censo
...
poblacion_area <- manzanas |> 
  select(CUT, COD_REGION, AREA_C, n_per) |> 
  filter(AREA_C == "URBANO") |> 
  group_by(CUT, COD_REGION) |>
  summarize(poblacion = sum(n_per)) |> 
  ungroup() |> 
  collect() |> 
  rename(comuna = CUT, region = COD_REGION)
...
future_map(1:16, \(.region) {
  ...
  # 
  # tamaño <- case_when(densidad_max > 10000 ~ .7,
  #                     densidad_max > 5000 ~ 1.2,
  #                     densidad_max <= 5000 ~ 1.6)
  
  # # tamaño de los puntos dependiendo de la región (podría ser por densidad...)
  # tamaño <- case_when(
  #   .region == 8 ~ .6,
  #   .region == 5 ~ .6,
  #   .region == 6 ~ .7,
  ...
  # si hay densidades pequeñas, mostrar con decimales
  ...
  ggplot() +
    ...
    geom_text(
      ...
      aes(x = 0, y = 0 - radio - 0.12, 
          label = glue("{label_number(accuracy = 0.1, suffix = ' km²')(superficie)}, {label_number()(poblacion)} hab.")),
    ...
    labs(title = "Densidad poblacional urbana por comuna",
  ...
  save_ggplot(
    plot = last_plot(),
    file = glue("graficos/urbana/densidad_urb_region_{.region}.jpg")
  )
